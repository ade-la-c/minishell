...
/*   Updated: 2021/11/25 16:34:48 by ade-la-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*.
static int	not_a_pipe(t_data *d, t_prog *prog, int *i, int *j)
{
...
// /*.
static int	no_pipe(t_data *d, t_prog *prog, int *i, int *j)
{
	while (*i < d->toklen && d->toks[*i].type != PIPE)
	{
		if (d->toks[*i].type >= MORE && d->toks[*i].type <= DLESS)
